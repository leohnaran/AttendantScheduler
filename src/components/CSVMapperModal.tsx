...
export default function CSVMapperModal({ data, onImport, onClose, language }: CSVMapperModalProps) {
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col border border-gray-200 dark:border-slate-800">
        ...
        <div className="p-8 overflow-y-auto max-h-[60vh]">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            {[
              { label: 'Full Name (Required)', key: 'nameIdx' as keyof Mapping, icon: 'fa-user' },
              { label: 'Role / Appointment', key: 'roleIdx' as keyof Mapping, icon: 'fa-user-tie' },
              {
                label: 'Key Man Name',
                key: 'keyManIdx' as keyof Mapping,
                icon: 'fa-users-gear',
              },
              {
                label: 'Congregation',
                key: 'congregationIdx' as keyof Mapping,
                icon: 'fa-church',
              },
            ].map((field) => (
              <div key={field.key}>
