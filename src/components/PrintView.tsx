...
import { Person, Assignment, Position, Shift } from '../types/models'
...
export default function PrintView({
  ...
  const [displayMode, setDisplayMode] = useState<'slips' | 'table'>('slips'); // 'slips' or 'table'

  const personnelMap = useMemo(() => new Map<number, Person>(personnel.map(p => [p.id, p])), [personnel]);

  const assignmentLookup = useMemo(() => {
    const lookup: Record<number, PrintAssignment[]> = {}
    ...
    Object.keys(lookup).forEach(pIdStr => {
  ...
  const getBrotherAssignments = (p: Person) => {
  ...
  const renderVolunteerTableCell = (p: Person, s: Shift) => {
            ...
            >
                ...
                <tbody>
                    {getBrotherAssignments(p).map((a, idx) => {
                      const pos = positions.find(x => x.name === a.posName || [x.name, ...positions.filter(p2 => p2.mirrorOf === x.id).map(m => m.name)].join(' + ') === a.posName);
                      return (
                        <tr
                            key={idx}
