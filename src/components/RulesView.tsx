...
import { RuleOptions } from '../types/models'

interface RulesViewProps {
  rules: RuleOptions;
  setRules: (rules: RuleOptions) => void;
  language: string;
}

export default function RulesView({ rules, setRules, language }: RulesViewProps) {
  const toggleRule = (key: keyof RuleOptions) => {
    const newVal = !rules[key];
    setRules({ ...rules, [key]: newVal });
  }

  const setSeverity = (key: keyof RuleOptions, val: string) => {
    setRules({ ...rules, [key]: val });
  }

  const setVal = (key: keyof RuleOptions, val: number) => {
    setRules({ ...rules, [key]: val });
  }
