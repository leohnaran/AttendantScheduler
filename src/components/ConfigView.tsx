...
import { Area, Shift, Position, Person, Tag, RuleOptions } from '../types/models'
...
export default function ConfigView({ language }: ConfigViewProps) {
  ...
  const blueprints = useAppStore((state) => state.blueprints)
  const updateState = useAppStore((state) => state.updateState)

  const setAreas = (val: Area[] | ((prev: Area[]) => Area[])) => 
    updateState({ areas: typeof val === 'function' ? val(areas) : val })
  
  const setPositions = (val: Position[] | ((prev: Position[]) => Position[])) => 
    updateState({ positions: typeof val === 'function' ? val(positions) : val })
  
  const setShifts = (val: Shift[] | ((prev: Shift[]) => Shift[])) => 
    updateState({ shifts: typeof val === 'function' ? val(shifts) : val })
  
  const setRules = (val: RuleOptions | ((prev: RuleOptions) => RuleOptions)) => 
    updateState({ rules: typeof val === 'function' ? val(rules) : val })
  
  const setBlueprints = (val: Blueprint[] | ((prev: Blueprint[]) => Blueprint[])) => 
    updateState({ blueprints: typeof val === 'function' ? val(blueprints) : val })

  const onConfigUpdate = (updates: Partial<any>) => updateState(updates)
  const [activeTab, setActiveTab] = useState('areas')

  const handleExportBlueprint = (blueprint: Blueprint) => {
    try {
  ...
  const [editAreaForm, setEditAreaForm] = useState<Area | null>(null)

  const [posForm, setPosForm] = useState<PositionForm>({
    name: '',
  ...
  const [editPosForm, setEditPosForm] = useState<Position | null>(null)
  ...
  const renderConstraintSelector = (form: any, setForm: (f: any) => void) => (
  ...
  const getConstraintLabel = (obj: any) => {
  ...
  const addArea = () => {
    ...
    setAreas([...areas, { id, ...areaForm } as Area])
    setAreaForm({
      name: '',
      capability: '',
      style: { backgroundColor: '#ffffff', color: '#000000' },
  ...
  const saveAreaEdit = async () => {
    if (!editAreaForm || !editingAreaId) return
  ...
  const addPosition = () => {
    ...
    setPositions([...positions, { id, ...posForm } as Position])
    setPosForm({
      ...posForm,
      name: '',
      limitType: '',
  ...
  const savePosEdit = () => {
    if (!editPosForm || !editingPosId) return
    setPositions(
        ...
        <div>
          <div className="bg-gray-50 p-4 rounded-xl border border-gray-200 mb-6 dark:bg-slate-800 dark:border-slate-700">
            ...
            <div className="mt-2 flex items-center">
              ...
              {renderConstraintSelector(areaForm, setAreaForm)}
            </div>
          </div>
                      ...
                      <td className="p-2">
                        {isEditing ? (
                          <div className="flex gap-1">
                            <input
                              type="color"
                              value={editAreaForm?.style?.backgroundColor || '#ffffff'}
                              onChange={(e) =>
                                setEditAreaForm({
                                  ...editAreaForm!,
                                  style: {
                            ...
                            <input
                              type="color"
                              value={editAreaForm?.style?.color || '#000000'}
                              onChange={(e) =>
                                setEditAreaForm({
                                  ...editAreaForm!,
                                  style: {
          ...
          <div className="bg-gray-50 p-4 rounded-xl border border-gray-200 mb-6 dark:bg-slate-800 dark:border-slate-700">
            ...
            <div className="flex justify-between items-center mt-4">
              <div className="flex items-center">
                ...
                {renderConstraintSelector(posForm, setPosForm)}
              </div>
              <button
                onClick={addPosition}
                className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-full text-sm font-bold shadow-lg shadow-blue-200 active:scale-95 transition-all dark:shadow-none"
                  ...
                  return (
                    <tr key={p.id}>
                      ...
                      <td className="p-2 text-xs">
                        {isEditing
                          ? renderConstraintSelector(editPosForm, setEditPosForm)
                          : displayConstraint}
                      </td>
