...
import { Person, Area, Position, Shift, Assignment } from '../types/models'

interface DepartmentViewProps {
  language: string;
}

interface OversightArea {
  type: string;
  area: Area | undefined;
  shiftId: string;
  shiftLabel?: string;
...
export default function DepartmentView({ language }: DepartmentViewProps) {
  ...
  const [selectedKeyManId, setSelectedKeyManId] = useState<string | number>(
  ...
  const renderSingleReport = (kmId: string | number) => {
    const km = personnelMap.get(typeof kmId === 'string' ? parseInt(kmId) : kmId)
    if (!km) return null

    // --- LOGIC: MY DIRECT TEAM ---
    // Include the Key Man himself in the direct team list as requested
    const myTeam = personnel
      .filter((p) => p.keyManId === km.id || p.id === km.id)
      .sort((a, b) => a.name.localeCompare(b.name))

    // --- LOGIC: MY AREAS OF OVERSIGHT ---
    const myOversightAreas: OversightArea[] = []
    ...
    const mirrorsMap = new Map<string, Position[]>() // sourcePosId -> array of mirror POS objects
    const mirrorsNameMap = new Map<string, string[]>() // sourcePosId -> array of mirror names
    ...
    const rotationalAssignmentsByMemberAndShift = new Map<string, string>()
    const auditoriumAssignmentsByMember = new Map<number, string>()
    ...
    const getTimeNotesForTeam = (team: Person[]) => {
    ...
    const getTimeNotesForOversight = (oversightList: OversightArea[]) => {
        const notes: TimeNote[] = []
                    ...
                    >
                        ...
                        <table className="w-full text-[10px] print:text-[11px]">
                            ...
                            {positions
                                ...
                                if (isMirror) {
                                    const sourcePos = sourcePosId ? positionsMap.get(sourcePosId) : null
                                ...
                                const assignedPerson = pid ? personnelMap.get(pid) : null
                                ...
                                return (
                                    ...
                                    <td className="py-1.5 text-right text-gray-400 print:text-black">
                                        {assignedPerson
                                        ? personnelMap.get(assignedPerson.keyManId || 0)?.name || 'None'
                                        : '-'}
                                    </td>
                                    </tr>
                                )
