...
import { Person, Position, Assignment, Area, Tag } from '../types/models'
...
export default function AssignmentCell({
  ...
  if (isMirror) {
    ...
    const mirrorId = mirrorVal ? (typeof mirrorVal === 'object' ? mirrorVal.id : parseInt(mirrorVal as unknown as string)) : null
  ...
  const assignedId = assignmentVal ? (typeof assignmentVal === 'object' ? assignmentVal.id : parseInt(assignmentVal as unknown as string)) : null
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <div className="relative w-full">
          <SearchableSelect
            value={assignedId || ''}
            onChange={(val: string) => onAssign(assignmentKey, val)}
          ...
          {quickFixes.length > 0 && (
            <div className="flex flex-wrap gap-1">
              ...
              {quickFixes.map((p) => (
                <button
                  key={p.id}
                  onClick={() => onAssign(assignmentKey, p.id.toString())}
