...
import { Tag, Person, Shift, Area } from '../types/models'
...
export default function TagsView({
  ...
  const [newTag, setNewTag] = useState<NewTagState>({
    name: '',
    restrictedShifts: [],
    restrictedAreas: [],
  })
  ...
  const toggleShiftNew = (shiftId: string) => {
  ...
  const toggleAreaNew = (areaId: string) => {
  ...
  const toggleShiftEdit = (tag: Tag, shiftId: string) => {
  ...
  const toggleAreaEdit = (tag: Tag, areaId: string) => {
  ...
  const updateTagName = (tag: Tag, newName: string) => {
    setTags(tags.map((t) => (t.id === tag.id ? { ...t, name: newName } : t)))
  }

  // --- BULK ASSIGNMENT ---
  const togglePersonTag = (personId: number, tagId: string) => {
