...
import { Person, RosterFormData, Area, Shift, Tag, Assignment } from '../types/models'
...
export default function RosterView({ onMerge, language }: RosterViewProps) {
  ...
  const setPersonnel = (val: Person[] | ((prev: Person[]) => Person[])) => 
    updateState({ personnel: typeof val === 'function' ? val(personnel) : val })
  
  const setAssignments = (val: Record<string, Assignment | null> | ((prev: Record<string, Assignment | null>) => Record<string, Assignment | null>)) => 
    updateState({ assignments: typeof val === 'function' ? val(assignments) : val })

  const initialCaps = useMemo(() => {
    const c: Record<string, boolean> = { keyman: false }
    areas.forEach((a) => {
        if (a.capability) c[a.capability] = true
    })
    return c
  }, [areas])
  ...
  const startEdit = (person: Person) => {
    ...
    const newCaps: Record<string, boolean> = { keyman: false }
    areas.forEach((a) => {
        if (a.capability) newCaps[a.capability] = false
    })
  ...
  const handleMergeAction = async (targetId: number) => {
  ...
  const savePerson = async () => {
    ...
    if (editingId !== null) {
      setPersonnel(
        personnel.map((p) =>
          p.id === editingId ? { ...p, ...newPerson } as Person : p,
        ),
      )
      ...
      Object.keys(newAssignments).forEach(key => {
        ...
        if (assignment && getAssignId(assignment) === editingId) {
          ...
          if (formData.unavailable.includes('all_day') ||
            formData.unavailable.includes(assignedShiftId) ||
            (assignedShiftId === 'all' && formData.unavailable.length > 0)) {
      ...
      if (affectedCount > 0) {
        if (await confirm(`${formData.name} is currently assigned to ${affectedCount} position(s) that conflict with this unavailability. Do you want to unassign them from those slots?`)) {
      ...
      setPersonnel([...personnel, { id: maxId + 1, ...newPerson } as Person])
      setFormData({
        name: '',
        congregation: '',
        role: 'Exemplary',
  ...
  const handleImportFinish = (mapping: { nameIdx: number, roleIdx: number, keyManIdx: number, congregationIdx: number }) => {
    if (!csvData) return
    const rows = csvData
    const dataRows = rows
      .slice(1)
    ...
    dataRows.forEach((row) => {
      ...
      if (existingIndex >= 0) {
        ...
        } as Person & { tempKeyManName?: string }
        updatedCount++
      } else {
        maxId++
        newPersonnel.push({
    ...
    const tempPersonnel = [...newPersonnel] as (Person & { tempKeyManName?: string })[]
    const keyMenToCreate = new Set<string>()
    ...
    // 3. Final Link pass
    const finalPersonnel = tempPersonnel.map((p) => {
      if (
        p.tempKeyManName &&
        ...
        if (km) {
            const { tempKeyManName, ...rest } = p
            return { ...rest, keyManId: km.id }
        }
      }
      const { tempKeyManName, ...rest } = p
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const personId = active.id as number;
    const targetKeyManId = over.id === 'unassigned' ? null : over.id as number;
