...
interface FindReplacementModalProps {
  slot: { pos: Position; shiftId: string } | null;
  personnel: Person[];
  ...
  shifts: Shift[];
  tags: Tag[];
  onAssign: (pos: Position, shiftId: string, personId: number, isDominoSwap?: boolean, dominoData?: any) => void;
  onClose: () => void;
  language: string;
}
...
interface Candidate extends Person {
  ...
  reason: string;
  isBusy: boolean;
  busyWhere: string | null;
  busySlot: { pos: Position; shiftId: string } | null;
  isDominoSwap?: boolean;
  dominoData?: any;
}
...
export default function FindReplacementModal({
  ...
  useEffect(() => {
    ...
    const busyMap = new Map<number, { busyWhere: string, busySlot: { pos: Position, shiftId: string } }>()
    ...
    const initialList: Candidate[] = personnel.map((p) => {
      ...
      } else if (p.unavailable && (p.unavailable.includes(shiftId) || (shiftId === 'all' && p.unavailable.includes('all_day')))) {
        isBusy = true
    ...
    const finalCandidates = initialList.map(p => {
      if (p.qualified && p.isBusy && p.busySlot) {
        let replacementForP: Candidate | null = null;
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-slate-700">
              {candidates.map((p) => (
                <tr
                  ...
                  onClick={() => p.qualified && (!p.isBusy || p.isDominoSwap) && onAssign(pos, shiftId, p.id, p.isDominoSwap, p.dominoData)}
                >
                  <td className="p-3">
